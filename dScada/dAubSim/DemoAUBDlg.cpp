...
BEGIN_MESSAGE_MAP(CDemoAUBDlg, CDialog)
    ...
    ON_BN_CLICKED(IDC_TNDSWAP, &CDemoAUBDlg::OnBnClickedTndSwap)
END_MESSAGE_MAP()
...
DWORD WINAPI DebugTask( LPVOID pData )
{
    ...
    CButton *b = (CButton *)dlg->GetDlgItem(IDC_TNDSWAP);

    // prikazi ime stanice i ime konfiguracije
    ...
    while( !ServerShutdown )
    {
        ...
        // ispisi trenutno vreme i stanje stanice
        sprintf( msg, "%s", ctime2str() );
...
void CDemoAUBDlg::StartScadaTasks()
{
    ...
    if( !(KomunikacioniTaskHnd = CreateThread( NULL, 0, CommunicationTask, NULL, 0, NULL)) )
        _Abort();
    SetThreadPriority( KomunikacioniTaskHnd, THREAD_PRIORITY_ABOVE_NORMAL );

    if( !(AkvizicioniTaskHnd = CreateThread( NULL, 0, AcquisitionTask, NULL, 0, NULL)) )
        _Abort();
...
BOOL CDemoAUBDlg::OnInitDialog()
{
    ...
    CString wtext;
    GetWindowText( wtext );
    wtext.AppendFormat( " %s", StnCfg.StaName );
    SetWindowText( wtext );

    LOGFONT lf;                                  // Used to create the CFont.
    memset(&lf, 0, sizeof(LOGFONT));
    lf.lfHeight = 13;     //-12;
    lf.lfWeight = FW_LIGHT; //FW_THIN;
...
void CDemoAUBDlg::OnBnClickedTndSwap()
{
}


void SetCloseApp( void )
{
